...
.profilePic{
	background-image: url(<?php echo "http://plychannel.com/Images/author?u=" . $user; ?>);
	background-size: 100%;
...
<div class="formInput">
	<span>Channel Banner: (1160 x 200)</span>
	<div class="Banner" id="channelBanner"><img id="bannerLoading" style="width:50px;display:none;" src="http://plychannel.com/Images/loading.gif" /></div>
</div>
<div class="formInput">
	<span>Profile Picture: (200 x 200)</span>
	<div class="profilePic" id="channelProfile" ><img id="profileLoading" style="width:50px;display:none;" src="http://plychannel.com/Images/loading.gif" /></div>
</div>
<div class="formInput">
...
<div class="formInput">
	...
	<textarea id="ChannelInfo"><?php echo $userData['About']; ?></textarea>
</div>

<script>
var uploadingProfile = false;
var uploadingBanner = false;
...
$("#ChannelInfo").change(function(){
	var about = $("#ChannelInfo").val();
	$.ajax({
	    type: "POST",
	    url: "http://plychannel.com/Include/changeChannelAbout.php",
	    data: { about: about },
	    success: function(data){
            $("#channelBanner").css("background-image", $("#channelBanner").css("background-image"));
...
$("#fileUploader").change(function(){
	if ($(this).val())
	{
		...
		$("#bannerLoading").css("display", "block");
...
$("#fileUploaderProfile").change(function(){
	if ($(this).val())
	{
		...
		$("#profileLoading").css("display", "block");
...
function uploadImage(url, file)
{
	...
	$.ajax({
	    ...
	    success: function(data){
            ...
            $("#channelBanner").css("background-image", $("#channelBanner").css("background-image"));
            $("#channelProfile").css("background-image", $("#channelProfile").css("background-image"));
        	if (url == "http://plychannel.com/Include/changeBackground.php")
