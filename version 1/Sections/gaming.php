<?php
  ...
  $query = mysql_query("SELECT * FROM `videos` WHERE `Category` = 'Gaming' AND `Uploaded` = '1' AND `Privacy` = 'a' AND `Time` > '".(time() - 2678400)."' ORDER BY `Views` DESC ,`Time` DESC LIMIT 15");
?>
<script type="text/javascript">
  ...
  function resizeSectionGame()
  {
    if ($(window).width() > 1184)
      $("#GameSection").css('width', 1150 + 'px');
    else if ($(window).width() > 976)
      $("#GameSection").css('width', 949 + 'px');
    else if ($(window).width() > 765)
      $("#GameSection").css('width', 715 + 'px');
    else
      $("#GameSection").css('width', 735 + 'px');
  }
</script>
<h3 style="font-size: 18px;font-weight: bold;color: #000;">
  <a href="http://plychannel.com/Gaming">Gaming</a>
</h3>
<div id="GameSection" style="margin-left:10px;margin-right:10px;overflow-x:scroll;overflow-y:hidden;height:220px;">
<table>
<tr>
<?php 
