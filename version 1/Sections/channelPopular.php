<?php
  ...
  $query = mysql_query("SELECT * FROM `videos` WHERE `Uploaded` = '1' AND `Privacy` = 'a' AND `Author` = '$user' ORDER BY `Views` DESC ,`Time` DESC LIMIT 15");
?>
<script>
	$(window).resize(function() {
    resizeChannelPopular();
  });
  $(document).ready(function (){
    resizeChannelPopular();
  });

  function resizeChannelPopular()
  {
    if ($(window).width() > 1184)
      $("#ChannelVideosPopular").css('width', 1150 + 'px');
    else if ($(window).width() > 976)
      $("#ChannelVideosPopular").css('width', 949 + 'px');
    else if ($(window).width() > 765)
      $("#ChannelVideosPopular").css('width', 715 + 'px');
    else
      $("#ChannelVideosPopular").css('width', 735 + 'px');
  }
</script>
<h3 style="font-size: 18px;font-weight: bold;color: #000;">
  Popular on <?php echo $channel['ChannelName']; ?>
</h3>
<div id="ChannelVideosPopular" style="margin-left:10px;margin-right:10px;overflow-x:scroll;overflow-y:hidden;height:220px;">
<table>
<tr>
<?php 
