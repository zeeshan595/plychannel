<?php
  ...
  $query = mysql_query("SELECT * FROM `videos` WHERE `Uploaded` = '1' AND `Privacy` = 'a' AND `Time` > '".(time() - 2678400)."' ORDER BY `Views` DESC ,`Time` DESC LIMIT 15");
?>
<script type="text/javascript">
	$(window).resize(function() {
    resizeSectionMostViewed();
  });
  $(document).ready(function (){
    resizeSectionMostViewed();
  });

  function resizeSectionMostViewed()
  {
    if ($(window).width() > 1184)
      $("#MostViewedSection").css('width', 1150 + 'px');
    else if ($(window).width() > 976)
      $("#MostViewedSection").css('width', 949 + 'px');
    else if ($(window).width() > 765)
      $("#MostViewedSection").css('width', 715 + 'px');
    else
      $("#MostViewedSection").css('width', 735 + 'px');
  }
</script>
<h3 style="font-size: 18px;font-weight: bold;color: #000;">
  Popular on plychannel
</h3>
<div id="MostViewedSection" style="margin-left:10px;margin-right:10px;overflow-x:scroll;overflow-y:hidden;height:220px;">
<table>
<tr>
<?php 
