<?php
  ...
  $query = mysql_query("SELECT * FROM `videos` WHERE `Uploaded` = '1' AND `Privacy` = 'a' AND `Author` = '$user' ORDER BY `Time` DESC LIMIT 15");
?>
<script>
  ...
  function resizeChannelLatest()
  {
    if ($(window).width() > 1184)
      $("#ChannelLatest").css('width', 1150 + 'px');
    else if ($(window).width() > 976)
      $("#ChannelLatest").css('width', 949 + 'px');
    else if ($(window).width() > 765)
      $("#ChannelLatest").css('width', 715 + 'px');
    else
      $("#ChannelLatest").css('width', 735 + 'px');
  }
</script>
<h3 style="font-size: 18px;font-weight: bold;color: #000;">
  Latest on <?php echo $channel['ChannelName']; ?>
</h3>
<div id="ChannelLatest" style="margin-left:10px;margin-right:10px;overflow-x:scroll;overflow-y:hidden;height:220px;">
<table>
<tr>
<?php 
