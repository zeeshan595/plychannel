...
linkSubscriptionButtons();
function linkSubscriptionButtons()
{
	$("[id=subButton]").each(function(){
		$(this).unbind("click");
		$(this).click(function(){
			if ($(this).attr("sub") == "0")
			{
				var subButt = $(this);
				var key = $(this).attr("number");
				var author = $(this).attr("author");
				$.ajax({
					url: "PHP/subscribe.php",
					type: 'post',
					data: { author: author, key: key },
					success: function(data){
						subButt.parent().prepend(data);
						subButt.remove();
						linkSubscriptionButtons();
					}
				});
...
if ($(".relatedVideos").height() > $(".videoDetails").height())
{
	$(".content").height($(".relatedVideos").height());
}
