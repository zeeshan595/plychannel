...
?>
		...
		<div class="bottomBar">
			...
			<div class="navBar">
				...
				<a href="http://plychannel.com/channelVideos?u=<?php echo $channelUser; ?>"><div style="border-bottom: #ff0000 3px solid; opacity: 1;">Videos</div></a>
		...
		<div class="content">
			<div class="section">
<?php
			$totalVideos = mysql_query("SELECT COUNT(`ID`) FROM `videos` WHERE `Author` = '$channelUser' AND `Uploaded` = '1' AND `Privacy` = 'a' LIMIT 1");
			$totalVideos = mysql_fetch_array($totalVideos);
			$totalVideos = $totalVideos[0];
			echo "<input type='hidden' value='$totalVideos' id='totalVideos' />";
			echo "<h3>All videos by $channelUser ($totalVideos)</h3>";
			$query = mysql_query("SELECT `Title`, `Description`, `ID`, `Views`, `Time`,`Length` FROM `videos` WHERE `Author` = '$channelUser' AND `Uploaded` = '1' AND `Privacy` = 'a' ORDER BY `Time` DESC LIMIT 50");
			echo mysql_error();
			...
			<a href="http://plychannel.com/watch?v=<?php echo urlencode(encrypt($row['ID'])); ?>">
			    <div class="videoThumb">
					...
					<div style="height:0; width: 0;">
						<div class="videoTime" style="top: -131px"><?php echo $row['Length']; ?></div>
					</div>
...
?>
		...
		$("#loader").click(function(){
			$.ajax({
				url: "PHP/getMoreAuthorVideos.php",
				type: 'post',
