...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<?php
			...
			$totalVideos = mysql_query("SELECT COUNT(`ID`) FROM `videos` WHERE  `Uploaded` = '1' AND `Privacy` = 'a' AND `Category` = '".mysql_real_escape_string($cat)."' LIMIT 1");
		...
		<div class="content">
			<div class="section">
			    <h3><?php echo $cat; ?></h3>
<?php
				$query = mysql_query("SELECT * FROM `videos` WHERE  `Uploaded` = '1' AND `Privacy` = 'a' AND `Category` = '".mysql_real_escape_string($cat)."' ORDER BY `Time` ASC LIMIT 50");
